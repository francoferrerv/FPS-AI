...
namespace Whisper.net.Native
{
    internal static class NativeLibraryLoader
    {
        internal static bool LoadNativeLibrary()
        {
            ...
            UnityEngine.Debug.Log(AppDomain.CurrentDomain.RelativeSearchPath);
            UnityEngine.Debug.Log(Path.GetDirectoryName(typeof(NativeMethods).Assembly.Location)!);
            ...
            if (!File.Exists(path))
            {
                path = Path.Combine(Directory.GetCurrentDirectory(), "runtimes", $"{platform}-{architecture}", $"whisper.{extension}");
                if (!File.Exists(path)) {
                    return false;
                }
            }
